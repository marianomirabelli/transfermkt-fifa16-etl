...
def fill_player_dataframe(df,player,player_id, first_name_id,lastnameid, ponderation):
        ...
        accessorycode1 = 0
        playerjointeamdate = parse_joined_on()  ## TODO: Fix date format
...
def get_players_by_team_id(tfk_club_id):
    url = "http://localhost:8000/clubs/%s/players" % tfk_club_id
    response = requests.get(url)
    return response.json()['players']

def get_team_profile_by_id(tfk_club_id):
    url = "http://localhost:8000/clubs/%s/profile" % tfk_club_id
    response = requests.get(url)
